    ...
    <section>
          ...
          {courses
            ?.map((course, index) => (
              <div className="card bg-base-100 shadow-xl relative" key={index}>
                <div>
                  <button
                    className="btn btn-circle btn-error text-white absolute top-2 left-2 shadow-lg"
                  ...
                  <button
                    className="btn btn-circle btn-info text-white absolute top-2 left-16 shadow-lg"
                    onClick={() => {
      ...
      {openModal && (
        <Modal
          ...
          content={<UpdateCourse course={selectCourse} />}
        />
      )}
    </section>
  );
